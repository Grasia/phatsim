...
import phat.devices.commands.CreateDoorSensorCommand;
import phat.devices.commands.CreatePresenceSensorCommand;
import phat.sensors.door.DoorStatePanel;
 ...
 * @author pablo
 */
public class FridgDoorSensorTest implements PHATInitAppListener, PHATCommandListener {
    ...
    public static void main(String[] args) {
        FridgDoorSensorTest test = new FridgDoorSensorTest();
    ...
    public void init(SimpleApplication app) {
        ...
        bodiesAppState.runCommand(new SetSpeedDisplacemenetCommand(bodyId, 1.0f));
        
        goToOpenTheFridge("Fridge1");
        ...
        createDoorSensor("Fridge1", "FridgeDoorSensor1");
    ...
    private void goToOpenTheFridge(final String fridge) {
        GoCloseToObjectCommand gtc = new GoCloseToObjectCommand(bodyId, fridge, new PHATCommandListener() {
            @Override
            public void commandStateChanged(PHATCommand command) {
                if (command.getState() == PHATCommand.State.Success) {
                    bodiesAppState.runCommand(new AlignWithCommand(bodyId, fridge));
                    bodiesAppState.runCommand(new OpenObjectCommand(bodyId, fridge));
                    bodiesAppState.runCommand(new SayASentenceBodyCommand(bodyId, "I'm hungry!"));
                }
            }
        });
    ...
    private CreateDoorSensorCommand createDoorSensor(String obj, String id) {
        CreateDoorSensorCommand cpsc = new CreateDoorSensorCommand(obj, id, this);
    ...
    @Override
    public void commandStateChanged(PHATCommand command) {
        if (command instanceof CreateDoorSensorCommand) {
            CreateDoorSensorCommand cpsc = (CreateDoorSensorCommand) command;
            Node psNode = devicesAppState.getDevice(cpsc.getDoorSensorId());
            if (psNode != null) {
                PHATDoorSensor psControl = psNode.getControl(PHATDoorSensor.class);
                if (psControl != null) {
                    DoorStatePanel psp1 = new DoorStatePanel();
                    psControl.add(psp1);
