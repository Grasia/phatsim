...
public class PHATConsole extends JFrame implements Controller
    ...
    static ImageIcon iconFor(String name)
        {
        return new ImageIcon(sim.display.Console.class.getResource(name));
        }
    ...
    public PHATConsole(final GUIState simulation)
        {
        ...
        if (isMacOSX()) 
            buttonBox.add(new JLabel("    "));  // move away from the scroll box
        ...
        */
                ...
                try
                    ...
                    if (l > Integer.MAX_VALUE || l < Integer.MIN_VALUE)  // outside of int range
                        Utilities.inform("Seed Will Be Truncated",
                            ...
                            "Not all the bits of this seed will be used.", phat.mason.gui.PHATConsole.this);           
        ...
        invokeInSwing(new Runnable() { public void run() { try { simulation.init(phat.mason.gui.PHATConsole.this); } catch (Exception e) { e.printStackTrace(); } } });
...
//            prioritySlider.setValue(appPrefs.getInt(THREAD_PRIORITY_KEY, systemPrefs.getInt(THREAD_PRIORITY_KEY, prioritySlider.getValue())));
    ...
    */
    ...
    public static void doQuit()
        ...
        synchronized(isQuittingLock)  // quitting causes closing, which in turn causes quitting...
            ...
            for(int x=0;x<entries.length;x++)
                if (entries[x] != null && entries[x] instanceof phat.mason.gui.PHATConsole)  // might occur if weak?  dunno
                    ((phat.mason.gui.PHATConsole)(((Map.Entry)(entries[x])).getKey())).doClose();
    ...
    public static void main(String[] args)
        ...
        sacrificial = isMacOSX();  // sacrificial  -- something to force Display2D.class to load
    ...
    static void buildClassList()
        ...
        try
            {
            InputStream s = sim.display.Console.class.getResourceAsStream("simulation.classes");
...
//            setRandomNumberGenerator(randomSeed);
    ...
    synchronized void pressPause(boolean shouldStartSimulationIfStopped)
        ...
        if (getPlayState() == PS_PLAYING) // pause
            ...
            ((PHATSimState) simulation.state).pausePHAT();
            ...
            ((PHATSimState) simulation.state).resumePHAT();
