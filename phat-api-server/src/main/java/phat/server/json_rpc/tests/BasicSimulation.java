 ...
 * @author pablo
 */
public class BasicSimulation implements PHATInitializer {

    public static void main(String[] args) {
        String[] a = {"-ml"};
        BasicSimulation sim = new BasicSimulation();
        PHATInterface phat = new JSONPHATInterface(sim, new ArgumentProcessor(a));
        phat.setSeed(0);
    ...
    public void initAgents(AgentConfigurator agentsConfig) {
        ...
        DiseaseManager dm = new DiseaseManager(relative);
        dm.setStage("E3");
        
        Symptom s1 = new Symptom("Tremors");
        s1.setCurrentLevel(Symptom.Level.MEDIUM);
        dm.add(s1);
        
        Symptom s2 = new Symptom("Rigidity");
        s1.setCurrentLevel(Symptom.Level.HIGH);
        dm.add(s2);
        
        relative.setDiseaseManager(dm);
    }
