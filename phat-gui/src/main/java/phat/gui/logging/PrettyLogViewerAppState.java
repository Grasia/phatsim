 ...
 * @author pablo
 */
public class PrettyLogViewerAppState extends AbstractAppState implements AutomatonListener, AgentListener {
	...
	public void initialize(AppStateManager stateManager, Application app) {
		...
		/*
		 * FileHandler fh = null; try { fh = new
		 * FileHandler(System.currentTimeMillis()+".log"); PHATLogFormatter formatter =
		 * new PHATLogFormatter(); fh.setFormatter(formatter); } catch
	...
	private void createAndShowGUI() {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				agentsAppState.getPHAInterface().getRootJFrame();
				...
				PrettyLogViewerPanel newContentPane = new PrettyLogViewerPanel(tableModel);
				newContentPane.setOpaque(true); // content panes must be opaque
				frame.add(newContentPane);
				agentsAppState.getPHAInterface().getRootJFrame().getContentPane().add(frame);
				// Display the window.
				// frame.pack();
			}
		});
	...
	public boolean isShown() {
		return frame.isVisible() && frame.getParent() != null;
	}
	...
	private void log(Automaton automaton, String state) {
		...
		if (automaton.getLeafAutomaton() == null) { // to show only leaf activities
			...
			String time = getTwoDigits(simTime.getHourOfDay()) + ":" + getTwoDigits(simTime.getMinute()) + ":"
					+ getTwoDigits(simTime.getSecond()) + "-" + getTwoDigits(simTime.getMonth()) + "/"
					+ getTwoDigits(simTime.getDayOfMonth()) + "/" + simTime.getYear();
